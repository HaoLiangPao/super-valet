...
| 角色 | 模型 | 职责 | 边界 |
| ---- | ---- | ---- | ---- |
| **创始人** | Fable（本体） | 产品判断、编排、读各角色报告摘要并验收、对投资人汇报 | 产品方向不外包 |
| **CTO** | Opus | 复杂技术决策端到端负责（架构、选型、实现、ADR 起草），向创始人交报告摘要 | 产品方向仍归创始人；ADR 经创始人确认 |
| **前端 / 后端工程师** | Sonnet（可多名） | 按 spec 实现功能、UI 还原、重构、写测试 | 偏离 spec 需在汇报中列明 |
| **QA** | Sonnet | 独立验证交付物（build/测试/浏览器走查/线上回归），出验收报告 | 只验证不修改；发现问题回报创始人 |
| **实习生 / 流水线** | Haiku | 机械转换、数据搬运、批量提取、格式化 | 绝不做任何设计决策 |

（2026-09-18 Hao 授权扩编两次：先 Opus 技术负责人 + 多 Sonnet；后升格 Opus CTO
+ 专职 Sonnet QA，并调高月度预算 —— 创始人转向编排与验收，读报告摘要放行。）
